  ...
  return (
    <div id={styles.component}>
      ...
      {Object.entries(data).map(([key, value], index) =>
        key !== "_id" ? (
          <div className={styles.editContainer} onDoubleClick={() => handleEdit(index)}>
            <div className={styles.editLabel}>{key}</div>
            { !uiFlag[index] ? 
            (<div className={styles.editPreview}>{value}</div>) : 
            (<input id={key} type="text" value={value} onChange={handleChange} className={styles.editTextArea}></input>)}
          </div> 
        ) : (
          <></>
        ),
